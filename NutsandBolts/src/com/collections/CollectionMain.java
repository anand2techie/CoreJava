...
*/		
	...
	private static void addElementsToSet(Set<String> customerLocationAsSet) {
		...
		customerLocationAsHashSet.add("Hyderabad"); //no exception but addition will be skipped
	}
	...
	private static void doListOperations() {
		//creating a List
		//List namesList=new List(); //Interface doesn't have a constructor. so this is not possible.
	...
	private static void useListIteratorToIterateList() {
		...
		//when u initialize list iterator you've to specify the point from where u need to iterate backwards
	...
	private static void useIteratorToIterateList() {
		...
		while(namesListItr.hasNext()) //check the condition until which this loop needs to run
		{
			String name=namesListItr.next(); //extracts the value one by one from the list
			//namesList.add("Jeeva");
			if(name.equals("Alex"))
			namesListItr.remove(); //remove will remove the elements that is pointing the current iteration
		}
	...
	private static void addELementAtSpecifiedIndex() {
		namesList.add(1, "Shiva"); //just an insertion; doesn't override. old element will go 1 position down
		
		System.out.println("List values after adding Shiva at 1st index: "+namesList); //this will print as an Object
	}
	...
	private static void addElementsToList() {
		namesList.add("Gokul"); //it will be added to 0th index namesList: 0 --> Gokul
		namesList.add("Ananth"); //it will be added to 1st index namesList: 0 --> Gokul; 1 --> Ananth etc....
	...
	private static List<String> createAListOfString() {
		...
		List<String> namesList=new ArrayList<String>(); //create 10 memories as an array
		return namesList;
	}
	...
	static void learnCollectionofObjects(){
		...
		//String customerName,int customerId,long customerAccBalance,String customerAddress,List<String> nominees
		...
		Customer charlesCustoomerObj=new Customer("Charles",123,10000l,"Chennai",Arrays.asList(nomineesArrayForCharles));
		Customer davidCustoomerObj=new Customer("David",124,1000l,"Hyderabad",Arrays.asList(nomineesArrayFoDavid));
	...
	static void learnSetOfObjects()
	{
		...
		//individual customer objects wth diff data
		Customer charlesCustoomerObj=new Customer("Charles",123,10000l,"Chennai",Arrays.asList(nomineesArrayForCharles));
		Customer davidCustoomerObj=new Customer("David",123,1000l,"Hyderabad",Arrays.asList(nomineesArrayFoDavid));
		Customer arunCustoomerObj=new Customer("Arun",125,40000l,"Bengaluru",Arrays.asList(nomineesArrayForArun));
		Customer divyaCustoomerObj=new Customer("Divya",126,20000l,"US",Arrays.asList(nomineesArrayForDivya));
